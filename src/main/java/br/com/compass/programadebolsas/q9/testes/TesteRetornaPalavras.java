...
public class TesteRetornaPalavras {
	...
	public static void main(String[] args) {
		...
		while (i < splited.length) {
			try (PreparedStatement pstm = con.prepareStatement("SELECT id,nome,descricao,desconto,dataInsercao FROM produtos_q9 WHERE nome LIKE ?", Statement.RETURN_GENERATED_KEYS)) {
				pstm.setString(1, "%"+splited[i]+"%");
				pstm.execute();

				try (ResultSet rs = pstm.getResultSet()) {
					while(rs.next()) {
						produtos.add(new Produto(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getDouble(4),rs.getBigDecimal(5),
